...
- [ ] **Добавить эндпоинт `GET /api/v1/health/`** - возвращает 200 OK
  - Используется для проверки доступности backend при старте приложения
  - Должен быть доступен без авторизации
  - Пример ответа: `{ "status": "ok" }`

### omnimap-sync

- [ ] **Добавить обработку ping/pong для WebSocket heartbeat**
  - При получении `{ "action": "ping" }` отвечать `{ "type": "pong" }`
  - Используется для определения разрыва соединения на клиенте
  - Интервал ping: 30 секунд
  - Если 2 pong пропущены подряд - клиент считает соединение разорванным
