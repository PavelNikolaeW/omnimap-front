...
│  │   │ A │ B │ C │        │  │    ⊞ Сетки: 2×2, 3×3, 4×4           │  │
│  │   ├───┴───┼───┤        │  │    ◫ Лейауты: Sidebar, Dashboard...  │  │
│  │   │   D   │ E │        │  │    ✦ Специальные: Kanban, Calendar   │  │
...
Пресеты организованы в галерею с категориями для удобного выбора:

### Категории

| Категория | Иконка | Описание |
|-----------|--------|----------|
| Динамические | ↔ | Адаптивные раскладки для произвольного числа блоков |
| Сетки | ⊞ | Простые равномерные сетки (2×2, 3×3, 4×4) |
| Лейауты | ◫ | Структурные раскладки (Sidebar, Dashboard, Holy Grail) |
| Специальные | ✦ | Пресеты с особой логикой (Kanban, Calendar, Gallery) |

### Список пресетов

| Пресет | Категория | Тип | Вместимость | Описание |
|--------|-----------|-----|-------------|----------|
| `horizontal` | Динамические | `layout: columns` | ∞ | Все блоки в горизонтальный ряд |
| `vertical` | Динамические | `layout: rows` | ∞ | Все блоки в вертикальный столбец |
| `2x2` | Сетки | cells | 4 блока | Простая сетка 2×2 |
| `3x3` | Сетки | cells | 9 блоков | Простая сетка 3×3 |
| `4x4` | Сетки | cells | 16 блоков | Простая сетка 4×4 |
| `sidebar` | Лейауты | cells | ∞ | Сайдбар слева + контент |
| `sidebar-right` | Лейауты | cells | ∞ | Сайдбар справа + контент |
| `dashboard` | Лейауты | cells | ∞ | Главный блок + виджеты + метрики |
| `holy-grail` | Лейауты | cells | ∞ | Header + Footer + 3 колонки |
| `kanban` | Специальные | cells | 3 блока | Доска: To Do, In Progress, Done |
| `gallery` | Специальные | cells | ∞ | Большие и маленькие карточки |
| `calendar` | Специальные | cells | 35 блоков | Календарь на месяц |

### Динамические пресеты

Динамические пресеты (`horizontal`, `vertical`) отличаются от остальных тем, что они:
- Автоматически адаптируются под любое количество дочерних блоков
- Используют `layout: 'rows'` или `layout: 'columns'` вместо `layout: 'cells'`
- Не требуют `layoutCells` конфигурации — раскладка вычисляется автоматически при рендере

```javascript
// Горизонтальный пресет сохраняет:
block.data = {
    layout: 'columns',  // Все дети в ряд
    layoutCells: null   // Не используется
}

// Вертикальный пресет сохраняет:
block.data = {
    layout: 'rows',     // Все дети в столбец
    layoutCells: null   // Не используется
}
```

**Примечание:** Пресеты с ограниченной вместимостью (2x2, 3x3, 4x4, kanban, calendar) отключаются если блоков больше чем может вместить пресет. Недоступные пресеты показываются полупрозрачными с крестиком.

### Конфигурация пресетов

В `LayoutEditorPanel.js` есть `PRESET_CATEGORIES` и `PRESET_CONFIG`:

```javascript
// Категории для группировки в галерее
...
const PRESET_CONFIG = {
    'horizontal': {
        ...
        layoutType: 'columns',  // Использует layout вместо layoutCells
        preview: ['┌─────────────┐', '│ □ □ □ □ ... │', '└─────────────┘']
    },
    'vertical': {
        maxBlocks: null,
        ...
        category: 'dynamic',
        label: 'Вертикальный',
        layoutType: 'rows',  // Использует layout вместо layoutCells
        preview: ['┌───┐', '│ □ │', '│ □ │', '│...│', '└───┘']
    },
    '2x2': {
        maxBlocks: 4,
        ...
        category: 'grids',
        label: '2×2',
        preview: ['┌─┬─┐', '├─┼─┤', '└─┴─┘']  // ASCII-превью
    },
    'dashboard': {
        maxBlocks: null,  // null = расширяемый
        minBlocks: 1,
